...
if(!empty($_POST['start']) && !empty($_POST['end']) && !empty($_POST['question_id']) ){
  ...
  join (select distinct(clients_contact.phone) ,point_of_interaction, company_id from clients_contact ) cc_sub on
  	cc_sub.phone = log_send_sms.phone_number
   where cc_sub.company_id = $company_id and  (date(created_at)=date(main.created_at) )   ) as 'column-1',
	count( main.phone_number ) as 'column-2',
	count( main.phone_number )*100/(select count(*) from log_send_sms
  join (select distinct(clients_contact.phone) ,point_of_interaction, company_id from clients_contact ) cc_sub on
  	cc_sub.phone = log_send_sms.phone_number
   where cc_sub.company_id = $company_id  and (date(created_at)= date(main.created_at) ) ) as  'column-3'
from
